...
P_o = [prob_sum(i, data_array)[1] for i in TiN]
P_e = [prob_sum(i, data_array)[2] for i in TiN]
P_c = [prob_sum(i, data_array)[3] for i in TiN]
logP = [prob_sum(i, data_array)[4] for i in TiN]

with open(f'{path}/posteriors_{method}_{ID}.txt', 'w') as file:
    file.write(f"P_n = {P_n}\n")
    file.write(f"P_o = {P_o}\n")
    file.write(f"P_e = {P_e}\n")
    file.write(f"P_c = {P_c}\n")
